...
class MainWindow(Frame):
    ...
    def __init__(self, parent, contoller):
        ...
        # define frames here
        left_frame = Frame(parent, background='gray37')
        left_frame.pack(side=TOP)
        right_frame = Frame(parent, background='gray15')
        right_frame.pack()
        bottom_frame = Frame(parent)
        bottom_frame.pack(side=BOTTOM)
        space_label6 = Label(bottom_frame, text="", background='gray15')
        space_label6.pack(side=BOTTOM)
        # bottom_frame = Frame(parent)
        # bottom_frame
        # define labels here
        space_label7 = ttk.Label(left_frame, text="", background='gray37')
        space_label7.pack(side=TOP)
        enter_label = ttk.Label(left_frame, text="Enter Link: ", background='gray37', foreground="white")
        enter_label.pack(side=LEFT)
        # define entry box here
        self.entry_link = Entry(left_frame, width="40")
        self.entry_link.pack(side=LEFT)
        space_label = ttk.Label(left_frame, text="    ", background='gray37')
        space_label.pack(side=LEFT)
        # space_label5 = Label(left_frame, text="lol   ")
        # space_label5.pack(side=BOTTOM)
        space_label5 = ttk.Label(right_frame, text="", background='gray15')
        ...
        # define buttons here
        download_button = ttk.Button(left_frame, text="Download")
        download_button.bind("<Button-1>", contoller.download_callback)
        download_button.pack(side=LEFT)
        space_label2 = ttk.Label(left_frame, text=" ", background='gray37')
        space_label2.pack(side=LEFT)

        space_label4 = ttk.Label(right_frame, text=" " * 25, background='gray15')
        space_label4.pack(side=TOP)
        rename_button = ttk.Button(right_frame, text="Rename")
        rename_button.bind("<Button-1>", contoller.rename_window_popup)
        rename_button.pack(side=LEFT)

        space_label3 = ttk.Label(right_frame, text=" " * 25, background='gray15')
